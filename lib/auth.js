        ...
        create: {
          after: async (user) => {
            try {
              ...
              // Check if a legacy Business exists with same email (migration case)
              business = await Business.findOne({ email: user.email.toLowerCase() });
              if (business) {
                // Link existing business to this Better Auth user
              ...
              console.error('[BetterAuth] Failed to link/create Business for user', user.id, err.message);
            }
          },
        },
      },
