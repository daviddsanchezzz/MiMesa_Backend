...
function initAuth(mongoClient) {
  ...
  _auth = betterAuth({
    database: mongodbAdapter(mongoClient, { dbName }),
    ...
    databaseHooks: {
      user: {
        create: {
          after: async (user) => {
            try {
              const existing = await Business.findOne({ ownerId: user.id });
              if (!existing) {
              ...
              console.error('[BetterAuth] Failed to create Business for user', user.id, err.message);
            }
          },
        },
      },
